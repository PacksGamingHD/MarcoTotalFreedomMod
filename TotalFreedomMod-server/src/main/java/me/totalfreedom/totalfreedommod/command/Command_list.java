...
public class Command_list extends FreedomCommand
{
    ...
    public boolean run(CommandSender sender, Player playerSender, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        for (Player player : server.getOnlinePlayers())
        {
            if (listFilter == ListFilter.ADMINS && !plugin.al.isAdmin(player) && !Command_vanish.vanished.contains(player))
            {
                continue;
            }

            if (listFilter == ListFilter.VANISHED_ADMINS && !plugin.al.isAdmin(player) && !Command_vanish.vanished.contains(player))
            {
                continue;
            }
